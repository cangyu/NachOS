...
public class UserProcess {
	...
	protected int handleOpen(int name) {
		...
		OpenFile file=UserKernel.fileSystem.open(fileName, false);//don't open with create mode
	 ...
	 */
	protected int handleRead(int fd,int buf,int cnt) {
		OpenFile file =descriptorManager.get(fd);
		...
		if(buf<0 || cnt<0) {
			Lib.debug(dbgProcess, "buf and cnt should't be negative");
			return (-1);
		}
		
		byte buf_tmp[] = new byte[cnt];
		int length=file.read(buf_tmp, 0, cnt);
		if(length<0) {
			Lib.debug(dbgProcess, "Fail to read from file");
			return (-1);
		}
		
		length=writeVirtualMemory(buf,buf_tmp,0,length);
		return length;
	}
	 ...
	 */
	protected int handleWrite(int fd, int buf,int cnt) {
		OpenFile file = descriptorManager.get(fd);
		...
		if(buf<0 || cnt<0) {
			Lib.debug(dbgProcess, "buf and cnt shouldn't be negative");
			return (-1);
		}
		
		byte buf_tmp[] = new byte[cnt];
		int length=readVirtualMemory(buf,buf_tmp,0,cnt);
		length=file.write(buf_tmp, 0, length);
		return length;
	}
	...
	protected int handleClose(int fd) {
		return descriptorManager.close(fd);
	}
	...
	protected int handleJoin(int processID,int status) {
		if(!childProcesses.contains(processID)) {
			Lib.debug(dbgProcess, "processID doesn't refer to a child process of current process");
			return (-1);
		}
...
//			Lib.assertNotReached("Unknown system call!");
	...
	public class DescriptorManager {
		...
		public OpenFile get(int fd) {
			if(fd<0 || fd>=maxFileDescriptorNum)
				return null;
