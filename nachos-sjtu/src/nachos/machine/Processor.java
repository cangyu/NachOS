...
public final class Processor {
	...
	/** Index of the global pointer register. */
	public static final int regGP = 28;
	...
	private class Instruction {
		...
		private void writeBack() throws MipsException {
			...
			/**
			 * if (test(Mips.OVERFLOW) && Lib.test(dst,31) != Lib.test(dst,32))<br>
			 * Bug identified by Tianqi Chen, Oct. 22nd, 2008<br>
			 * Bug fixed by Xiangru Chen, Sep. 14th, 2009
			 */
			if (test(Mips.OVERFLOW)
					&& Lib.test(dst, 1L << 31) != Lib.test(dst, 1L << 32))
				throw new MipsException(exceptionOverflow);
