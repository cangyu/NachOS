...
public class Folder extends File
{
  ...
  public void save ()
  {
    acquireWrite();
    size=4;
    for(FolderEntry fe : entrySet.values())
    	size+=fe.name.length()+1+4;
    ...
    pos+=4;
    for(FolderEntry fe: entrySet.values()) {
    	string2Buffer(fe.name,buffer,pos);
    	pos+=fe.name.length()+1;
    	Disk.extInt(fe.addr, buffer, pos);
    	pos+=4;
    }
  ...
  public void load ()
  {
    ...
    int rd_cnt = read(pos,buffer,0,buffer.length);
    Lib.assertTrue(rd_cnt==buffer.length);
    pos+=4;
