...
public class LotteryScheduler extends PriorityScheduler {
	...
	public boolean increasePriority() {
		...
		int tmp_prio=getPriority(thread);
		if(tmp_prio==priorityMaximum)
			return false;
		
		setPriority(thread,tmp_prio+1);
	...
	protected class LotteryThreadState extends PriorityScheduler.ThreadState {
		...
		private int getEffectivePriority(HashSet<LotteryThreadState> set) {
			...
			if(set.contains(this))//deead lock detected!
				return priority;
