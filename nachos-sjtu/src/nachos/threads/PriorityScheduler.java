...
public class PriorityScheduler extends Scheduler {
	...
	protected class ThreadState {
		...
		public int getEffectivePriority() {
			...
			return getEffectivePriority_Helper(new HashSet<ThreadState>());
		}
		
		/**
		 * Helper function to resolve the effective priority.
		 * @param visited 
		 * @return the effective priority.
		 */
		protected int getEffectivePriority_Helper(HashSet<ThreadState> visited) {
			//Task 1.5
			...
			for(PriorityQueue queue : donationQueue) {
				if(queue.transferPriority) {
					for(KThread thread : queue.waitQueue) {
						visited.add(this);
						int p = getThreadState(thread).getEffectivePriority_Helper(visited);
						visited.remove(this);
						effectivePriority=Math.max(p,effectivePriority);
					}
				}
			}
